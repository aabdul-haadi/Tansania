...
const destinations = [
  ...
  { name: 'Botswana', slug: 'botswana', desc: 'Entdecken Sie die wilde Luxus-Welt', img: 'https://picsum.photos/seed/safari-botswana/600/800', hint: 'botswana wildlife' },
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
          {destinations.map((dest, idx) => (
            <motion.div key={dest.slug} initial={{ opacity: 0, scale: 0.95 }} animate={mounted ? { opacity: 1, scale: 1 } : {}} viewport={{ once: true }} transition={{ delay: idx * 0.05 }} className="group relative aspect-[4/5] rounded-3xl overflow-hidden cursor-pointer bg-muted shadow-sm hover:shadow-xl transition-all duration-500">
              <Image src={dest.img} alt={dest.name} fill className="object-cover transition-transform duration-700 group-hover:scale-110" data-ai-hint={dest.hint} />
