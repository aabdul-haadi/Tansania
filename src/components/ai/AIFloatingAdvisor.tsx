  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-4">
      ...
      <AnimatePresence>
        {showTeaser && !isOpen && (
          <motion.div
            ...
            className="bg-white px-6 py-4 rounded-[2rem] shadow-2xl border border-primary/20 relative max-w-[240px] hidden md:block cursor-pointer"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={cn(
              "bg-white rounded-[2.5rem] shadow-2xl border border-border/50 flex flex-col overflow-hidden",
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="relative flex items-center gap-2">
                    <div className="relative flex-grow">
                      <Input
                        ...
                        className="h-14 pl-5 pr-12 rounded-2xl border-muted bg-[#fdfcfb] shadow-inner focus:ring-primary/20 text-xs font-bold"
                      />
                      <Button type="button" size="icon" variant="ghost" className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-xl text-muted-foreground hover:text-primary">
      ...
      <motion.button
        ...
        onClick={() => { setIsOpen(true); setShowTeaser(false); }}
        className={cn(
          "w-16 h-16 md:w-20 md:h-20 rounded-[2rem] bg-secondary text-white shadow-2xl flex items-center justify-center relative overflow-hidden transition-all duration-500",
