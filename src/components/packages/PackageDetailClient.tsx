...
export function PackageDetailClient({ pkg }: PackageDetailClientProps) {
  ...
  return (
    <div className="bg-[#fdfcfb] min-h-screen font-normal">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 gap-3">
                        {[
                          { label: "Reisedauer", val: `${pkg.durationDays} Tage, Flüge inklusive` },
                          { label: "Unterkünfte", val: "Handverlesene Tented Lodges & Boutique Hotels" },
                          { label: "Exklusivität", val: "Privat-Safari mit eigenem Guide & Jeep" },
                          { label: "Reisezeit", val: "Ganzjährig buchbar" }
                        ].map((item, i) => (
                          <div key={i} className="flex flex-col">
                            <p className="text-secondary font-bold text-sm">{item.label}</p>
      ...
      {/* 04 ITINERARY - COMPACTED SPACING */}
                ...
                <AccordionTrigger className="p-0 hover:no-underline [&>svg]:hidden">
                  <div className="relative w-full h-24 rounded-3xl overflow-hidden bg-secondary flex items-center px-8 md:px-12">
                    <div className="relative z-10 text-left">
                      <h3 className="font-headline text-lg md:text-2xl text-white tracking-tight">Safari Abenteuer • Tag {gIdx * 5 + 1}-{gIdx * 5 + group.length}</h3>
                    ...
                    <div key={dIdx} className="flex gap-6 md:gap-10 text-left items-start">
                      ...
                      <div className="flex-1 bg-white rounded-3xl border border-border/40 overflow-hidden shadow-sm flex flex-col md:flex-row">
                        <div className="flex-1 p-6 md:p-10 space-y-4">
                          ...
                          <h4 className="font-headline text-xl md:text-3xl mb-4 tracking-tight">{day.title}</h4>
      ...
      <section ref={hotelsRef} className="py-12 md:py-20 bg-white scroll-mt-20 border-t border-border/40">
        <div className="container mx-auto px-4 max-w-7xl text-center space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              { title: "Boutique Safari-Lodges", desc: "Luxuriöse Zeltcamps und Lodges mitten in der Wildnis. Jede Unterkunft verbindet authentisches Safari-Feeling mit höchstem Komfort.", img: "https://images.unsplash.com/photo-1646668072507-b2215b873c70?q=80&w=800" },
              { title: "Sansibar Beach Retreat", desc: "Ihr privates Strandparadies auf Sansibar. Exklusives Boutique-Resort direkt am schneeweißen Sandstrand.", img: "https://images.unsplash.com/photo-1590001158193-79013ac7fa77?q=80&w=800" }
            ].map((hotel, i) => (
              <div key={i} className="bg-white rounded-[2rem] overflow-hidden border border-border/40 shadow-sm text-left group">
                <div className="relative aspect-video overflow-hidden">
                  <Image src={hotel.img} alt="Lodge" fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
                </div>
                <div className="p-8 space-y-4">
                  <h3 className="font-headline text-2xl font-medium">{hotel.title}</h3>
