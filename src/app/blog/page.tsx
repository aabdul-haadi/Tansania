...
export default function BlogListing() {
  ...
  // Registry Sync: Fetch live journal entries from Firestore
  const blogsQuery = useMemoFirebase(() => {
    if (!firestore) return null;
    // Mandatory filter for non-admin list operations
    return query(
  ...
  const { data: firestorePosts, isLoading } = useCollection(blogsQuery);

  const filteredPosts = (firestorePosts || []).filter(p => {
  ...
  return (
    <div className="bg-[#fdfcfb] min-h-screen font-normal">
      {/* 01 COMPACT DARK HERO - Standardized Casing */}
        ...
        <div className="container mx-auto px-4 max-w-7xl relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-end gap-10">
            <div className="max-w-3xl text-left">
              ...
              >
                <Compass className="w-3.5 h-3.5" /> Journal
              </motion.div>
              <motion.h1 
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.1 }}
                className="text-white font-headline text-4xl md:text-7xl font-normal leading-tight tracking-tight uppercase"
              >
          ...
          <div className="lg:col-span-8 space-y-12">
            {isLoading ? (
              <div className="py-40 text-center space-y-4">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground animate-pulse">Syncing Journal...</p>
              </div>
              ...
              <div className="py-40 text-center border-2 border-dashed rounded-[3rem] bg-muted/20">
                ...
                <h3 className="text-secondary font-headline text-xl md:text-2xl uppercase">No Entries Found</h3>
                <p className="text-[10px] font-bold uppercase text-muted-foreground mt-2 tracking-widest">Versuchen Sie es mit einem anderen Filter.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-12">
                {filteredPosts.map((post, idx) => (
                  <BlogCard key={post.id} post={post as any} featured={idx === 0 && !search && activeCategory === 'Alle'} />
                ))}
              </div>
            )}
          </div>
